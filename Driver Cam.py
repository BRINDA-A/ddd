...
label = Label(frame, text="Driver Drowsiness detection",bg='light blue',font=('Times 25 bold'))
label.pack(side=TOP)
...
def webrec():
   ...
   op=cv2.VideoWriter('output.avi',fourcc,11.0,(640,480))
   while True:
      ret,frame=capture.read()
      gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
      cv2.imshow('frame',frame)
...
def webdetRec():
   ...
   eye_glass = cv2.CascadeClassifier('haarcascade_eye_tree_eyeglasses.xml')
   fourcc=cv2.VideoWriter_fourcc(*'XVID') 
   op=cv2.VideoWriter('output_with_boxes.avi',fourcc,9.0,(640,480))
